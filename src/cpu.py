...
class CPU:
    ...
    def doInterruptRequest(self):
        ...
        if self.InterruptRequest == 0x4E:  # N for NMI
            self.registers['PC'] = self.RAM.read(0xFFFA) | (
                self.RAM.read(0xFFFB) << 8)
            self.z = 1
        elif self.InterruptRequest == 0x52:  # R for RESET
            self.registers['PC'] = self.RAM.read(0xFFFC) | (
                self.RAM.read(0xFFFD) << 8)
        elif self.InterruptRequest == 0x49 and not self.statusFlags[
                'i']:  # I for INTERRUPT MASK
            self.registers['PC'] = self.RAM.read(0xFFFE) | (
                self.RAM.read(0xFFFF) << 8)
        ...
        while True:
            ...
            if self.clock.value >= 113:
                ...
                if 0 <= self.scanline < 240 and not self.console.PPU.VBLANK.status:
                    ...
                    self.console.PPU.renderer.display.DEBUG_LAYER.text(
                        "Cycles {0} | FPS: {1}".format(cyclesCounter,
                                                       fpsCounter))
                    cyclesCounter = 0
